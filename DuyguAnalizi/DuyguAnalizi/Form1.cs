...
namespace DuyguAnalizi
{
    public partial class Form1 : Form
    {
        ...
        {
            pictureBox2.Image = sobel((Image)pictureBox1.Image.Clone());
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            /*int x1, y1,x2,y2;
            x1 = pictureBox1.Width / 2 - 50;
            x2 = x1 + 100;
            y1 = pictureBox1.Height / 3 - 50;
            y2 = y1 + 100;

            Rectangle rect = new Rectangle(pictureBox1.Location.X + x1, pictureBox1.Location.Y + y1, pictureBox1.Location.X + x2, pictureBox1.Location.Y + y2);
            Pen greenpen = new Pen(Color.Green, 3);
            Graphics.FromImage(pictureBox1.Image).DrawRectangle(greenpen, rect);
              */
        ...
        private void YCbCr_Click(object sender, EventArgs e)
        {
            ...
            /*
            cr_start = Convert.ToInt16(numericUpDown_cr_start.Value);
            cr_end = Convert.ToInt16(numericUpDown_cr_end.Value);
            cb_start = Convert.ToInt16(numericUpDown_cb_start.Value);
            cb_end = Convert.ToInt16(numericUpDown_cb_end.Value);
            */
            cr_start = 140;
            cr_end = 170;
            cb_start = 105;
            ...
            for (int i = 1; i < bb.Width - 1; i++)
                for (int j = 1; j < bb.Height - 1; j++)
                {
                    ...
                    if ((cr > cr_start && cr < cr_end) && (cb > cb_start && cb < cb_end))//nice result for good image                                     
                    {
                        ...
                        //bb1.SetPixel(i, j, Color.FromArgb(bb.GetPixel(i, j).R, bb.GetPixel(i, j).G, bb.GetPixel(i, j).B));
            ...
            /*
            #region drawing rectangle found above
            for (int i = min_x; i <= max_x; i++)
            {
                bb1.SetPixel(i, min_y, Color.Red);
                bb1.SetPixel(i, max_y, Color.Red);
            }
            for (int i = min_y; i <= max_y; i++)
            {
                bb1.SetPixel(min_x, i, Color.Red);
                bb1.SetPixel(max_x, i, Color.Red);
            }
            ...
            */
            ...
            /* Bitmap bbbb = new Bitmap((max_x - min_x), (max_y - min_y));
             ...
             for (int i = min_x; i < max_x; i++)
             {
                 ...
                 {
                     bbbb.SetPixel(i - min_x, j - min_y, bb1.GetPixel(i, j));
                 }
             }
             cut_x_start += min_x;
             cut_y_start += min_y;
             ...
             //r = (bm.Height * bm.Width);
             //MessageBox.Show(avg.ToString()+" "+r.ToString());
        ...
        private Bitmap dilation(Bitmap bmp)
        {
            ...
            //Inversion.LogicalOperator processing = new Inversion.LogicalOperator();
        ...
        {
            /*
            numericUpDown_cr_start.Value = 140;
            numericUpDown_cr_end.Value = 170;
            numericUpDown_cb_start.Value = 105;
            numericUpDown_cb_end.Value = 150;
             */
        }
        ...
        public Image face(Image Im)
        {
            ...
            /*
                    for (i = 0; i < b.Height; i++)
                    {
                        b.SetPixel(x_st,i,Color.Red);
                        b.SetPixel(x_en, i, Color.Red);
                    }
            */
            int t;
            t = x_en - x_st;
        ...
        /*  public void conect_lip(int x, int y, int w, int h)
          {

              if (limit_x_start < x && x < limit_x_end && limit_y_start < y && y < limit_y_end)
              {
                  if (x < lip_wx)
                      lip_wx = x;
                  if (x > lip_wy)
                  ...
                  if (x - 1 >= 0)
                      ...
                      {
                          big[x - 1][y] = big[x][y];
                          conect_lip(x - 1, y, w, h);
                      }
                  if (y - 1 >= 0)
                      if (Bit.GetPixel(x, y - 1).B == 0 && Bit.GetPixel(x, y - 1).R == 0 && Bit.GetPixel(x, y - 1).G == 0 && big[x][y - 1] == 0)
                      {
                          big[x][y - 1] = big[x][y];
                          conect_lip(x, y - 1, w, h);
                      }
                  if (x + 1 < w)
                      if (Bit.GetPixel(x + 1, y).B == 0 && Bit.GetPixel(x + 1, y).R == 0 && Bit.GetPixel(x + 1, y).G == 0 && big[x + 1][y] == 0)
                      {
        ...
        /* private void eyelocal()
            {
                ...
                Bit = new Bitmap(b);
                limit_x_start = xe1 / 3;
                limit_x_end = 2 * (w - xe2) / 3 + xe2;
                limit_y_start = lip_h1 - (h / 10);
                limit_y_end = h - 2;
                lip_wx = w - 1;
                lip_wy = 0;
                for (j = po1; j <= po2; j++)
                    if (b.GetPixel(j, lip_h1).R == 0 && b.GetPixel(j, lip_h1).B == 0 && b.GetPixel(j, lip_h1).G == 0)
                        if (big[j][lip_h1] == 0)
                        {
                            big[j][lip_h1] = 1;
                            conect_lip(j, lip_h1, w, h);
                        }

                po2 = (((w - xe2) / 3) + xe2);
                po1 = 2 * (xe1 / 3);

                int lip_x = lip_h1, lip_y = lip_h1;
                ////// for (j = po1; j <= po2; j++)
                //////    b.SetPixel(j,lip_h1,Color.Red);

                for (i = lip_h1 - 1; i >= ye2; i--)
                {
                    int cc = 0;
                    for (j = po1; j <= po2; j++)
                    {
                ...
                MessageBox.Show(spq + "," + ys2 + "," + ye2 + "," + xe2 + "," + w);

                b = new Bitmap(pictureBox1.Image);

                Bitmap B1 = new Bitmap(xe1 + 1, (ye1 - ys1) + 1);
                for (i = 0; i <= xe1; i++)
                    for (j = ys1; j <= ye1; j++)
                    {
                ...
                B1.Save(@"c:\image need\1.jpg");

                Bitmap B2 = new Bitmap((w - xe2), (ye2 - ys2) + 1);
                for (i = xe2; i < w; i++)
                    for (j = ys2; j <= ye2; j++)
                    {
             ...
             lip_x = lip_x - 2;
                ...
                Bitmap B3 = new Bitmap(lip_wy - lip_wx, lip_y - lip_x);
                for (i = lip_wx; i < lip_wy; i++)
                    for (j = lip_x; j < lip_y; j++)
                        B3.SetPixel(i - lip_wx, j - lip_x, b.GetPixel(i, j));
        ...
        public void conntected1()
        {
            ...
            Bitmap bbbb = new Bitmap(w, h + h / 10);
            Bitmap pic1 = (Bitmap)pictureBox1.Image.Clone();

            for (int i = min_x; i < max_x; i++)
            {
                for (int j = min_y; j < max_y + h / 10; j++)
                {
                    bbbb.SetPixel(i - min_x, j - min_y, pic1.GetPixel(i, j));
        ...
        public void conntected2()
        {
            ...
            for (int i = 0; i < Bit.Width; i++)
            {
                for (int j = 0; j < Bit.Height; j++)
                {
                    ...
                    if (!visited[i][j] && (Bit.GetPixel(i, j).R == 0 && Bit.GetPixel(i, j).G == 0 && Bit.GetPixel(i, j).B == 0))//&& c>135)
                    {
                        countt[count_region] = 0;
                        //MessageBox.Show(i+" "+j+" ");
                        /*save_j[count_region] = j;
                        save_i[count_region] = i;
                        keep_index[count_region] = count_region;*/
                        BFS(i, j);
            ...
            /*
            for (int i = count_region-1; i >0; i--)
            {
                for (int j = 1;j<=i ;j++ ) 
                { 
                    if (countt[j] > countt[j - 1])
                    {
                        ...
                        countt[j] = countt[j - 1];
                        countt[j - 1] = tmp;
                        save_j[j-1]^=save_j[j]^=save_j[j-1] ;
                        save_i[j - 1] ^= save_i[j] ^= save_i[j - 1];
                        keep_index[j - 1] ^= keep_index[j] ^= keep_index[j - 1];
            ...
            {
                if (point_y_max < save_j[i])
                {
                    point_y_max = save_j[i];
                    btmp = save_i[i];
                    point_y = keep_index[i];             
                }
                 MessageBox.Show(countt[i].ToString()+" "+save_j[i].ToString());
            }


            max_bit = big[btmp][point_y_max];*/

            //MessageBox.Show(point_y.ToString() + " ------ " + point_y_max.ToString()+" "+max_bit.ToString());
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            richTextBox1.Text = "1. Adım :\n" + "Bir görüntü seçiniz";
            richTextBox2.Text = "2. Adım :\n" + "Ten rengi dönüştürür";
            richTextBox3.Text = "3. Adım :\n" + "En büyük bağlantılı bölgeyi bulur";
            richTextBox4.Text = "4. Adım :\n" + "Yüz olma olasılığını kontrol eder";
        }
        ...
        */
        ...
        private void button_bezier_lips_click(object sender, EventArgs e)
        {
           ...
           double tot_pixel = bm.Height * bm.Width;
           Color color = new Color();
           visited = new bool[bm.Width + 5][];

           #region initialization of visited boolean array
           for (int i = 0; i < bm.Width + 5; i++)
           {
               visited[i] = new bool[bm.Height + 5];
           ...
           pictureBox2.Invalidate();
            ...
            /***Bitmap b = new Bitmap((Image)pictureBox_lips.Image.Clone());
            Bitmap ba = new Bitmap(skin_color(b));
                
            pictureBox_lips_bezier.Image = (Image)ba.Clone();
                
            ba = new Bitmap(connected_lips((Bitmap)pictureBox_lips_bezier.Image.Clone()));
            pictureBox_lips_bezier.Image = (Image)ba;
                
            ba = new Bitmap(bezier((Bitmap)pictureBox_lips_bezier.Image.Clone()));***/
            lips_bezier();
        ...
        private void button_pc4_pc1_Click(object sender, EventArgs e)
        {
            ///////  pictureBox1.Image = (Bitmap)pictureBox4.Image;

        }

        private void button_test_Click(object sender, EventArgs e)
        {
            /*double avg_r, avg_g, avg_b,cb,cr;
            double total,max_cb,min_cb,max_cr,min_cr;
            string file = file_name;
            
            total = 0;
            avg_b = avg_g = avg_r = 0;

            Bitmap bmp =(Bitmap) Image.FromFile(file).Clone();
            Color color = new Color();
            min_cb = min_cr = 255;
            max_cb = max_cr = 0;
            ...
            MessageBox.Show(avg_r.ToString()+" , "+avg_g.ToString()+" , "+avg_b.ToString());
            MessageBox.Show(min_cr.ToString()+" "+ max_cr.ToString()+" , "+min_cb.ToString()+" "+max_cb.ToString());
             * */

            pictureBox2.Image = white((Image)pictureBox1.Image.Clone());
        }

        private void button5_to_1_Click(object sender, EventArgs e)
        {
            //////   pictureBox1.Image = (Bitmap)pictureBox5.Image.Clone();
        ...
        private void button_right_bezier_Click(object sender, EventArgs e)
        {
            //////   right_bezier_eye();
        }
        ...
        {
            string person = textBox_person.Text;
            if (person != "")
            {
                ...
                string connectionString = @"Provider=Microsoft.Jet.OLEDB.4.0;Data Source=D:\db.mdb";
                ...
                AI = new int[10];
                Bitmap b = new Bitmap(pictureBox_lips_bezier.Image);
                Point_detection(b);
                int lip_width, lip_hight;
                lip_width = AI[0];
                lip_hight = AI[1];
                lip_hight = (lip_hight * 100) / lip_width;
                lip_width = 100;
                int emo = 0, min = 100;
                int pt = 0;
                if (emotion[0] != 0)
                {
                    ...
                    while (reader.Read())
                    {
                        pt = Convert.ToInt16(reader[38].ToString());
                    }
                    reader.Close();
                    if (Math.Abs(pt - lip_hight) < min)
                    {
                        emo = 0;
                        min = Math.Abs(pt - lip_hight);
                    }
                }
                if (emotion[1] != 0)
                {
            ...
            else
            {
                ...
                Bitmap b = new Bitmap((Image)pictureBox_lips_bezier.Image.Clone());
                int w, h;
                w = b.Width;
                h = b.Height;
                ...
                int w1, hh1, w2, hh2, result_eye = -1;

                b = new Bitmap((Image)pictureBox_left_eye.Image.Clone());
                ...
                b = new Bitmap((Image)pictureBox_right_eye.Image.Clone());
                ...
                h4 = y2 - h3;
                h5 = h3 - y1;
                ////textBox1.Text = Convert.ToString(x4 - x3) + "," + Convert.ToString(y2 - y1) + "," + h4+","+h5+","+result_eye;
                double q1, q2, q3, q4, q5, q6;
                q1 = Convert.ToDouble(x4 - x3);
                q2 = Convert.ToDouble(y4 - y3);
                q1 = q1 * q1 + q2 * q2;
                q1 = Math.Sqrt(q1);
                q2 = Convert.ToDouble(x1 - x3);
                q3 = Convert.ToDouble(y1 - y3);
                q2 = q2 * q2 + q3 * q3;
                q2 = Math.Sqrt(q2);
                q3 = Convert.ToDouble(x1 - x4);
                q4 = Convert.ToDouble(y1 - y4);
                q3 = q3 * q3 + q4 * q4;
                q3 = Math.Sqrt(q3);
                q4 = (q1 + q2 + q3) / 2;
                q5 = Math.Sqrt(q4 * (q4 - q1) * (q4 - q2) * (q4 - q3));
                q5 = 2 * q5 / q1;

                q2 = Convert.ToDouble(x2 - x3);
                q3 = Convert.ToDouble(y2 - y3);
                q2 = q2 * q2 + q3 * q3;
                q2 = Math.Sqrt(q2);
                q3 = Convert.ToDouble(x2 - x4);
                q4 = Convert.ToDouble(y2 - y4);
                q3 = q3 * q3 + q4 * q4;
                q3 = Math.Sqrt(q3);
                q4 = (q1 + q2 + q3) / 2;
                q6 = Math.Sqrt(q4 * (q4 - q1) * (q4 - q2) * (q4 - q3));
                q6 = 2 * q6 / q1;


                h1 = Convert.ToInt16(q1);
                h2 = Convert.ToInt16(q5 + q6);
                h4 = Convert.ToInt16(q6);
                h5 = Convert.ToInt16(q5);

                //////textBox1.Text += h1 + "," + h4 + "," + h5 + "," + result_eye;
                ...
                if (h4 < h5 && h1 > h2)
                {
                    ...
                    return "Surprised";
                else if (h1 < (3.5 * Convert.ToDouble(h2)))
                {   ////&& h5 * 3 > h4
                    if (result_eye == -1 || result_eye > 2)
                        return "Smile";
                    else
                        return "Surprised";
                }
                else if (h1 <= (Convert.ToDouble(h2) * 5.5))
                    return "Smile";
                else
                    return "Normal";
            ...
            int flag_man = 0;
              ...
              if (person != "" && flag_man == 1)
              {
                 ...
                 Bitmap b = new Bitmap(pictureBox1.Image);
                  ...
                  lip_h = (lip_h * 100) / lip_width;
                  lip_hh = (lip_hh * 100) / lip_width;
                  lip_width = 100;
                  int emo = 0, min = 200;
                  int pt = 0, pt1 = 0;
                  if (emotion[0] != 0)
                  {
                      ...
                      {
                          pt = Convert.ToInt16(reader[38].ToString());
                          pt1 = Convert.ToInt16(reader[43].ToString());
                      }
                      reader.Close();
                      if (Math.Abs(pt - lip_h) + Math.Abs(pt1 - lip_hh) < min)
                      {
                          emo = 0;
                          min = Math.Abs(pt - lip_h) + Math.Abs(pt1 - lip_hh);
                      }
                  }
                  if (emotion[1] != 0)
                  {
            ...
            /*
                  ...
                  if (w2 > hh2)
                  {
                      if (w2 < hh2 * 10)
                      {
                          if (result_eye == -1)
                              result_eye = w2 / hh2;
                          else if (result_eye > w2 / hh2 || result_eye == 1)
                              result_eye = w2 / hh2;
                      }
                  }
                  ...
                  */
            //////textBox1.Text += h1 + "," + h4 + "," + h5 + "," + result_eye;
            //// MessageBox.Show(h1+","+h2+","+h4+","+h5+","+result_eye);
            /*

                  if (h1 <= h2 || (h1 < h2 * 2 && h4 < h5 * 2))
                      return "Surprised";
                  else if (h1 < (3.5 * Convert.ToDouble(h2)))
                  {   ////&& h5 * 3 > h4
                      if ((h4 > h5 * 2) || result_eye >= 4)
                          return "Smile";
                      else
                          return "Surprised";
                  }
                  else if (h1 <= (Convert.ToDouble(h2) * 5.5) && h4 > h5)
                      return "Smile";
                  else if (h4 >= 4 * h5 && h1 <= (Convert.ToDouble(h2) * 7.0) && h4 >= h5)
                      return "Smile";
                  else if (h4 < h5 && h1 > h2)
                  {
                      return "Sad";
        ...
        {
            //System.Diagnostics.Process.Start("clien.exe");
            /// label_decision.Text = "Wait a bit";
            //// label_decision.Text = take_decion();
        }
        ...
        private void button_auto_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (FileInfo file in files)
                {
                    ...
                    ///// eye_and_lip_region_separatoin();

                    ////pictureBox1.Image =(Image) pictureBox3.Image.Clone();
                    my_eye();
                    ////pictureBox_left_eye.Image = (Image)pictureBox2.Image.Clone();

                    ///// pictureBox1.Image = (Image)pictureBox4.Image.Clone();
                    my_eye();
                    ////// pictureBox_right_eye.Image = (Image)pictureBox2.Image.Clone();
                    //left_eye2();
                    //right_eye2();
                    /////left_bezier_eye();
                    ///////right_bezier_eye();
                    ///// pictureBox_left_eye.Image.Save(@"C:\experiment\left eye0\" + file.Name );
                    /////  pictureBox_right_eye.Image.Save(@"C:\experiment\right eye0\" + file.Name );
                    //////  pictureBox_left_bezier.Image.Save(@"C:\experiment\left eye bezier0\" + file.Name );
                    //////  pictureBox_right_bezier.Image.Save(@"C:\experiment\right eye bezier0\" + file.Name );

                    //Bitmap b = new Bitmap((Image)pictureBox5.Image.Clone());
                    //Bitmap ba = new Bitmap(skin_color(b));
                    //pictureBox1.Image = (Image)ba;

                    //ba = new Bitmap(big_conect(ba));
                    //pictureBox1.Image = (Image)ba;



                    //lip_location((Bitmap)pictureBox1.Image.Clone());
                    //pictureBox_lips.Image = pictureBox1.Image;
                    //lips_bezier_draw();
                    //pictureBox_lips.Image = pictureBox5.Image;
                    //pictureBox_lips.Image.Save(@"C:\experiment\lips0\" + file.Name );
                    //pictureBox_lips_bezier.Image.Save(@"C:\experiment\lips bezier0\" + file.Name );


                    //MessageBox.Show(take_decion());
                }
            }
            catch (Exception exp)
            {
        ...
        private void lips_bezier_draw()
        {
            ...
            /*
            Bitmap b = new Bitmap((Bitmap)pictureBox_lips.Image.Clone());
        ...
        {
            ////a file to share commonly both this interface and
            // robot interface program to share to take decision
            string FILE_NAME = @"c:\input.txt";

            //if (File.Exists(FILE_NAME)) // if file exist delete the file
            ...
            /****************** decision part *******************/
            /*
             if (label_decision.Text.Equals("Wait a bit"))
                 MessageBox.Show("Unable to give command. No emotion.");
             else if (label_decision.Text.Equals("Ambiguous"))
                 MessageBox.Show("Ambiguous Emotion. Can not generate command.");
             else if (label_decision.Text.Equals(""))
                 MessageBox.Show("No decision to generate command.");
             else if (label_decision.Text.Equals("Sad"))
             {

                 tw.WriteLine("4");
                 MessageBox.Show("Generating robot command");
                 //System.Diagnostics.Process.Start("clien.exe");
             }
             else if (label_decision.Text.Equals("Surprised"))
             {
                 tw.WriteLine("3");
                 MessageBox.Show("Generating robot command");
                 //System.Diagnostics.Process.Start("clien.exe");
             }
             else if (label_decision.Text.Equals("Normal"))
             {
             ...
             else if (label_decision.Text.Equals("Smile"))
             {
             ...
             tw.Close();
             System.Diagnostics.Process.Start(@"c:\clien.exe");
             */
        }
